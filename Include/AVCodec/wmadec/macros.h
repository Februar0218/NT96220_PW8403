...
// ======================================================
// MIPS (Boston) Platform
// ======================================================
#elif defined(WMA_TARGET_MIPSFP)
...
//ARM Platform
// ======================================================
#elif defined(WMA_TARGET_MS_ARM)
...
#    endif
    ...
    #define PLATFORM_SPECIFIC_INLINE
#    ifdef __arm
        // really a placehold - check what it should be in ARM's compiler
#        define INLINE __inline
#    else
#        define INLINE __forceinline
#    endif
...
// override in platform specific macros_xxx.h as appropriate
#define WMA_MEMCLR_ALIGNED(p,c) memset ((p), 0, (c))
#define WMA_MEMCLR(p,c) memset ((p), 0, (c))
...
#define BPX_ABS(a) (((a)<0) ? -(a) : (a))
    ...
    INLINE I32 quickRand(tRandState* ptRandState)
    {
        ...
        iTemp = ((I32)(ptRandState->uiRand)); 

        // *1.25 to make the range (-1.25, 1.25) 
        iTemp = (iTemp>>2) + (iTemp>>4);
...
#elif defined(WMA_TARGET_MIPS) && !defined(_MIPS64) //64bit mips compiler doesn't support inline assembly well

// Output target
#ifdef BUILD_INTEGER
#ifdef COMMONMACROS_OUTPUT_TARGET
#pragma COMPILER_MESSAGE("-----MACROS.H: Compiling for MIPS integer Casio Target")
#endif // COMMONMACROS_OUTPUT_TARGET
#else
#ifdef COMMONMACROS_OUTPUT_TARGET
...
#elif defined(WMA_TARGET_MIPSFP) && !defined(_MIPS64) //64bit mips compiler doesn't support inline assembly well

// Output target
#ifdef BUILD_INTEGER
#ifdef COMMONMACROS_OUTPUT_TARGET
#pragma COMPILER_MESSAGE("-----MACROS.H: Compiling for MIPSFP integer Boston Target")
#endif // COMMONMACROS_OUTPUT_TARGET
#else
#ifdef COMMONMACROS_OUTPUT_TARGET
#pragma COMPILER_MESSAGE("-----MACROS.H: Compiling for MIPSFP floating point Boston Target")
#endif // COMMONMACROS_OUTPUT_TARGET

#endif

#include "..\mips\macros_mipsfp.h"
...
#include "..\sh4\macros_SH4.h"
    ...
    // Output target
#    ifdef COMMONMACROS_OUTPUT_TARGET
#        pragma COMPILER_MESSAGE("-----MACROS.H: Compiling for ARM (StrongARM) Target")
#    endif // COMMONMACROS_OUTPUT_TARGET

//#    include "..\armx\macros_arm.h"
...
#endif
{
    ...
    while (pbSrc < pbSrcEnd) {
        pbSrc++;
        b2 = *pbSrc;
        *pbDst = (U8)((b1 << iBitStartSrc) | (b2 >> iShiftDown));
        b1 = b2;
        pbDst++;
    }
}
...
static void Norm4FastFloat( FastFloat* pfflt )
    {   // use the faster Norm4FastFloatU when you know the value is positive
        register UInt uiF = (UInt)(abs((int)pfflt->iFraction));
        register Int iFB = 0;
        if ( uiF == 0 )
        {
            pfflt->iFracBits = 0;
...
static void Norm4FastFloatU( FastFloat* pfflt )
    ...
    INLINE
#endif  
static void ffltMultiply( FastFloat *pfflt, const FastFloat* const pffltA, const FastFloat* const pffltB )
    {
        pfflt->iFraction = MULT_HI( pffltA->iFraction, pffltB->iFraction );
        pfflt->iFracBits = (pffltA->iFracBits + pffltB->iFracBits - 31);
        Norm4FastFloat( pfflt );
    }
#   define FASTFLOAT_MULT(p,a,b) ffltMultiply((p),(a),(b))

#ifdef HITACHI
    #pragma inline(ffltMultiplyInt)
#else   
    INLINE
#endif  
static void ffltMultiplyInt( FastFloat *pfflt, const FastFloat* const pffltA, const Int iB )
    {
        FastFloat ffltB;
        ffltB.iFracBits = 0;
        ffltB.iFraction = iB;
        Norm4FastFloat( &ffltB );
        ffltMultiply( pfflt, pffltA, &ffltB );
    }
#   define FASTFLOAT_MULT_INT(p,a,b) ffltMultiplyInt((p),(a),(b))

#ifdef HITACHI
    #pragma inline(ffltAdd)
#else   
    INLINE
#endif  
static FastFloat ffltAdd( const FastFloat ffltA, const FastFloat ffltB )
    {
        FastFloat ffltP;
        int iFBdiff = (int)(ffltA.iFracBits - ffltB.iFracBits);
        if ( iFBdiff > 0 ) {
            ffltP.iFracBits = ffltB.iFracBits-1;
            ffltP.iFraction = (ffltB.iFraction>>1) + (ffltA.iFraction >> (iFBdiff+1));
        } else {
            ffltP.iFracBits = ffltA.iFracBits-1;
            ffltP.iFraction = (ffltA.iFraction>>1) + (ffltB.iFraction >> (1-iFBdiff));
        }
...
#ifdef HITACHI
    #pragma inline(ffltDivideInt)
#else   
    INLINE
#endif  
static void ffltDivideInt( FastFloat *pfflt, const FastFloat ffltA, const Int iB )
    {
        pfflt->iFracBits = ffltA.iFracBits;
        pfflt->iFraction = ffltA.iFraction / iB;
        Norm4FastFloat( pfflt );
    }
#   define FASTFLOAT_DIV_INT(p,a,b) ffltDivideInt((p),(a),(b))
...
static FastFloat FastFloatFromFloat(Float flt) {
    ...
    INLINE
#endif
static Float FloatFromFastFloat( const FastFloat fflt )
    {
...
#   define DOUBLE_FROM_FASTFLOAT(fflt) ((double)fflt.iFraction/(1<<fflt.iFracBits))
    ...
    INLINE
#endif  
static void FastFloatFromU64(FastFloat *pfflt, const U64 u64, const Int cExp) 
    {
        ...
        pfflt->iFracBits = iExp+cExp-32;
#if defined(PLATFORM_OPTIMIZE_MINIMIZE_BRANCHING)
        pfflt->iFraction = (U32)((u64<<iExp)>>32);
#else
        pfflt->iFraction = (iExp>32) ? (U32)(u64<<(iExp-32)) : (U32)(u64>>(32-iExp));
#endif
    }   
#define FASTFLOAT_FROM_U64(pfflt,u64,exp) FastFloatFromU64(pfflt,u64,exp)
...
#   define FASTFLOAT_MULT(p,a,b) (*(p))=((*(a))*(*b))
#   define FASTFLOAT_MULT_INT(p,a,b) *(p)=((*(a))*(b))
#   define FASTFLOAT_ADD(a,b) ((a)+(b))
#   define FASTFLOAT_DIV(a,b) ((a)/(b))
#   define FASTFLOAT_DIV_INT(a,b) ((a)/(b))
#   define DOUBLE_FROM_FASTFLOAT(fflt) ((double)fflt)
#   define FASTFLOAT_FROM_U64(pflt,u64,exp) *(pflt)=(((Float)(u64))/(((U64)1)<<exp))

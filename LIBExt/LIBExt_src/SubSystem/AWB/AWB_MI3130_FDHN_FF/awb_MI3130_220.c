...
AWB_MODE AWB_getMode(UINT32 Idx)
{
    ...
    case _IMGWBMode_Fluorescent1:
        return AWB_MODE_FLUORESCENT1;

    case _IMGWBMode_Fluorescent2:
        return AWB_MODE_FLUORESCENT2;

    case _IMGWBMode_Sunset:
        return AWB_MODE_SUNSET;

    case _IMGWBMode_BirthdayParty:
        return AWB_MODE_BIRTHDAYPARTY;
...
UINT32 AWB_WriteDebugInfo(UINT32 Addr)
{
    ...
    for(j = 0; j < AWBALG_WIN_Y; j++)
    {
        ...
        for (i = 0; i < AWBALG_WIN_X; i++)
        {
            Addr += sprintf((char *)Addr,"%3ld %3ld %3ld\t",    (AwbInfo.RAcc[j*AWBALG_WIN_Y+i] * AwbInfo.KRgain) / 100,

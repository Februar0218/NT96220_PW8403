...
{
    UINT32 RB_AdjM = 310,RB_Adjm = 250;
    UINT32 R_AdjRng = 100;
    UINT32 B_AdjRng = 40;
    UINT32 ratio=0;
...
void AWB_autoProc(AWBALG_INFO *Info)
{
    ...
    else
    {
        ...
        {
            RGain = (MwbTAB[AWB_MODE_CLOUDY][0] * 3 + OldRGain*7) / 10;//the CT of cloudy is lower the daylight in adpk
            GGain = AWB_DEF_GGAIN;
            BGain = (MwbTAB[AWB_MODE_CLOUDY][1] * 3 + OldBGain*7) / 10;
            OldRGain = RGain;
            OldGGain = GGain;
            OldBGain = BGain;
        }
...
{
    STR_AAA_CA_PARA SourceImg = {0};
    UINT32 NewRGain,NewBGain;
    ER ER_code;
    AWB_MODE Mode;
    PImageAlgInfo ImgInfo;
    ImgInfo = UI_GetParameter();
    Mode = AWB_getMode(ImgInfo->WBMode);

    if(Mode == AWB_MDOE_AUTO)
    {
        //re-cal sourceImg CA result
        SourceImg.bitdepth = DataBit;
        SourceImg.DramAddr = addr;
        SourceImg.DramLofs = Lineoffset;
        SourceImg.fmt = PROGRESSIVE;
        SourceImg.ImgHsize = Hsize;
        SourceImg.ImgVsize = Vsize;
        SourceImg.Sub_rate = SubRate;
        SourceImg.Win_x = AWBALG_WIN_X;
        SourceImg.Win_y = AWBALG_WIN_Y;
        SourceImg.Cfapat = CFA;
        SourceImg.R_Data = AwbInfo.RAcc;
        SourceImg.G_Data = AwbInfo.GAcc;
        SourceImg.B_Data = AwbInfo.BAcc;
        aaa_GetCAResult(&SourceImg);
        //cal new gain, use preview awb threshold
        ER_code = AWBALG_ChkImg(&AwbInfo, &NewRGain, &NewBGain);
        if(ER_code == AWBALG_OK)
        {
            RGain = (NewRGain * 7 + OldRGain*3) / 10;
            //GGain = (NewGGain * 3 + OldGGain*7) / 10; //Ggain = orignal
            BGain = (NewBGain * 7 + OldBGain*3) / 10;
        }
        else
        {
            //do nothing, use preview color gain
        }
    }
    else
    {
        //do nothing ,use mwb color gain
    }
}
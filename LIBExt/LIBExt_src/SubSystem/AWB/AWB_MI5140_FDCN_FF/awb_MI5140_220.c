...
void AWB_autoProc(AWBALG_INFO *Info)
{
    ...
    if(ER_code == AWBALG_OK)
    {
        ...
        RGain = (NewRGain * 5 + OldRGain*5) / 10;
        GGain = (NewGGain * 5 + OldGGain*5) / 10;
        BGain = (NewBGain * 5 + OldBGain*5) / 10;
    }
    else
    {
...
//        AE_GetPrvAEArg(&prviso,&prvexpt,&iris);
   ...
   /* if (1)
    ...
    SenFP_ShowOSDString(message,50,120,4);
    }    */

...
#include "LensMotor_S5K3E2.h"
#include "Sensor.h"

static void motorS5K3E2_zoom_setState(MOTOR_ZOOM_ACT state, UINT32 param1);
...
//--------------------------------------------------------------------------------------------------------------------------
void motorS5K3E2_zoom_setState(MOTOR_ZOOM_ACT state, UINT32 param1)
{
       UINT32 step;
    step = param1;
           debug_msg("%s:%d-%d\r\n",__func__,step,state);
       ...
       if(step)
       {
        ...
        Sensor_WriteReg(0x3131,(step>>8)& 0x3);

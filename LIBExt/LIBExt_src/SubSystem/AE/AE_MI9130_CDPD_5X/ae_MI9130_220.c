...
//follow zheng         
//post Gamma                               
/*static UINT32 EVTable[(Max_EvValue + 1)] = {64,68,72,76,81,85,90,95,101,107,113,120,128,
                                        136,145,154,163,174,184,194,204,212,221,
                                        230,238};*/
//Pre Gamma                               
static UINT32 EVTable[(Max_EvValue + 1)] = {44,48,52,57,62,67,74,80,86,95,105,116,128,
                                        145,160,180,200,225,250,275,300,330,360,
                                        390,420};
...
void AE_Process (void)
{
    ...
    if (AEUartMsg == ENABLE)
    {
        AE_ERR("%3d %3d %8d %3d %7d, %3d, %d, %d %d\r\n", AeInfo.LumY, AEAlg_GetPrvLV(), AEAlg_GetPrvEVValue(),
                    PrvNewAeArg.ISOGain, PrvNewAeArg.ExpoTime, AEAlg_GetAdjExpectY(), PrvNewAeArg.Iris,AEAlg_GetStbCnt(),uiImeY);
        debug_err(("----------------------------\r\n"));
    }
}
...
{
    Addr += sprintf((char *)Addr, "AE Stable = \t%ld\r\nExpectY = \t%ld\r\nAdd_Y = \t%ld\r\nLumY = \t%ld\r\n",
                                            AEAlg_GetStbCnt(),AEAlg_GetAdjExpectY(), AEAlg_GetAdjY(), AeInfo.LumY);
...
void AE_getCapTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_SCENE nScene, AE_ANTISHAKE nShakeLevel, AE_INFO *AeInfo)
{
    ...
    if(nFlicker == AE_FLICKER_50HZ)
    {
        switch(nISO)
        {
            ...
            case AE_ISO_AUTO:
            default:
                AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_NoramlCapture50;
                break;
        }
    }
    else//if(nFlicker==AE_FLICKER_60HZ)
    {
        switch(nISO)
        {
            ...
            case AE_ISO_AUTO:
            default:
                AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_NoramlCapture60;
                break;
        }
    }
    ...
    if (nShakeLevel != AE_ANTISHAKE_LV0)
    {
        if(nFlicker == AE_FLICKER_50HZ)
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_AntiShake50;
        }
        else
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_AntiShake60;
        }
    }

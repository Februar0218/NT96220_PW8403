...
void AE_Process (void)
{
    ...
    UINT32 ISOGain4SmoothLum, PreEV, CurEV;
    ...
    PreEV = AEAlg_GetPrvEVValue();
    ISOGain4SmoothLum = PrvNewAeArg.ISOGain;

    //calculate ae
    AEAlg_Converge(&AeInfo, &PrvNewAeArg, &CapNewAeArg);

    CurEV = AEAlg_GetPrvEVValue();
    //ISOGain4SmoothLum = (ISOGain4SmoothLum*PreEV/CurEV);
    ISOGain4SmoothLum = (ISOGain4SmoothLum*(PreEV+CurEV)/(CurEV*2));
    ...
    if (AEUartMsg == ENABLE)
    {
        AE_ERR("Y=%3d LV=%3d EV=%8d ISO=%3d ET=%7d, ExptY=%3d, Iris=%d, Stable=%d ImeY=%d\r\n", AeInfo.LumY, AEAlg_GetPrvLV(), AEAlg_GetPrvEVValue(),

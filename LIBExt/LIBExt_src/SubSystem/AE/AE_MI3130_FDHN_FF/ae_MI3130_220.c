/*
    ...
    @file       ae_MI3130_220.c
    @ingroup        AE_MI3130_FDHN_FF
    @note       Nothing.
...
static UINT32 EVTable[(Max_EvValue + 1)] =
    {
    // adpk said that EV-3 was too dark & EV+3 was too bright,
    // and asks us to "correct" the problem, so re-calculate the values
    // to meet their need. [NTK#RandyHo]<2012/07/09>
    50, 53, 56, 59,
    63, 71, 79, 87,
    94, 95, 97, 99,
    100, 107, 114, 121,
...
/*
    // standard value-according to APEX
        13,16,18,20,
        25,31,35,40,
        50,63,71,79,
        100,126,141,159,
...
void AE_Process (void)
{
    ...
    if (AEUartMsg == ENABLE)
    {
        AE_ERR("%3d %3d %8d %3d %7d, %3d, %d, %d %d %d\r\n", AeInfo.LumY,
                                                        ...
                                                        AEAlg_GetPrvEVValue(),
                                                        PrvNewAeArg.ISOGain,
                                                        PrvNewAeArg.ExpoTime,
                                                        AEAlg_GetAdjExpectY(),
                                                        PrvNewAeArg.Iris,
                                                        AEAlg_GetStbCnt(),
                                                        uiImeY,
                                                        expoinfo.uiExpoRatio);
        debug_err(("----------------------------\r\n"));
    }
...
UINT32 GetCenterOverExpoRatio(UINT32 HistMaxNum, UINT32 *Hist)
{
    ...
    //#NT#2012/05/14#KS Hung -begin
    //#NT#refer from other project, else the "cnt" will be a random number.
...
UINT32 GetFnoRatio(AEALG_IRIS_IDX Idx)
{
    ...
    FN = 30; // Lens_Aperture_GetFNO(iris,Lens_Zoom_GetSection());//FF lens [NTK#RandyHo]<2012/05/08>
...
{
    // begin -re-write ifs to switch()  [NTK-SZ#RandyHo]<2012/03/08>
    switch(Idx)
    {
...
/*
    ...
    return AE_SCENE_AUTO;
*/
    // end -re-write ifs to switch()    [NTK-SZ#RandyHo]<2012/03/08>
}

AE_FLICKER AE_getFlickerType(UINT32 Idx)
{
    if (Idx == _POWERFreq_50Hz)
    {

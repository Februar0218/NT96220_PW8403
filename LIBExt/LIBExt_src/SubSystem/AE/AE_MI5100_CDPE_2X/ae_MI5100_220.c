...
/*static UINT32 EVTable[(Max_EvValue + 1)] = {13,16,18,20,25,31,35,40,50,63,71,79,100,
                                        ...
                                        635,800};*/
//Pre Gamma                               
static UINT32 EVTable[(Max_EvValue + 1)] = {34,37,41,45,48,52,58,63,67,74,82,91,100,
                                        113,125,141,156,176,195,215,234,258,281,305,328};
...
void AE_getCapTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_SCENE nScene, AE_ANTISHAKE nShakeLevel, AE_INFO *AeInfo)
{
   ...
   if(nFlicker == AE_FLICKER_50HZ)
    ...
    if (nShakeLevel != AE_ANTISHAKE_LV0)
    {
        if(nFlicker == AE_FLICKER_50HZ)
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Shake50;
        }
        else
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Shake60;
        }
    }

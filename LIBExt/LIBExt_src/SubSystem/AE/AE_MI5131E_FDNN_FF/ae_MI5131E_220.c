...
#include "sie_lib.h"
#include "Lens.h"
#define FDNN_CAP    ENABLE
#if FDNN_CAP
UINT32 FDNN_CapExpt;
UINT32 Special_RAWY = 50;
#endif
...
static UINT32 EVTable[(Max_EvValue + 1)] = {13,16,18,20,25,31,35,40,50,63,71,79,100,
                                        ...
                                        635,800};
*/
static UINT32 EVTable[(Max_EvValue + 1)] = {31,32,33,34,   35 ,45,   50 ,72,   65 ,86,90,93,    100  ,
                                        107,110,114,   140 ,148,   160 ,160,   180 ,190,200,
                                        635,800};
...
{
#if FDNN_CAP    //follow HDNN
    FDNN_CapExpt = CapNewAeArg.ExpoTime;
    if( (AEAlg_GetPrvEVValue() <= (AeInfo.MinEV * 11 / 10))&&(AeInfo.LumY<=Special_RAWY))
    {
        FDNN_CapExpt = FDNN_CapExpt* Special_RAWY / AeInfo.LumY;
    }
    if(FDNN_CapExpt > 500000)
        FDNN_CapExpt = 500000;
    *ExpT = FDNN_CapExpt;
#else
...
#if FDNN_CAP
    Addr += sprintf((char *)Addr, "real exptY = \t%ld\r\n",
                                            Special_RAWY);
#endif
...
#if FDNN_CAP
    Addr += sprintf((char *)Addr, "real cap expt = \t%ld\r\n",
                                            FDNN_CapExpt);
#endif
    return Addr;
}
...
void AE_getCapTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_SCENE nScene, AE_ANTISHAKE nShakeLevel, AE_INFO *AeInfo)
{
    ...
    if (nShakeLevel != AE_ANTISHAKE_LV0)
    {
        if(nFlicker == AE_FLICKER_50HZ)
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Cap50_antishake;
        }
        else
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Cap60_antishake;
        }
    }

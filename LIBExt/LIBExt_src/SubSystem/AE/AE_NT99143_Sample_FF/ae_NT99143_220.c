...
static UINT32 EVTable[(Max_EvValue + 1)] = {13,16,18,20,25,31,35,40,50,63,71,79,100,
                                        ...
                                        635,800};
*/
static UINT32 EVTable[(Max_EvValue + 1)] = {36,44,48,51,58,65,69,72,79,86,90,93,100,107,110,114,121,128,131,135,142,149,152,156,163};

static AEALG_EVENT_TAB AeAlgEventTab =
{
    GetOverExpoRatio,
    GetCenterOverExpoRatio,
    GetFnoRatio,
    NULL
};
...
void AE_Process (void)
{
	...
	{
		OB_Value = 0xE;
		//OB_Value = 0x10 - ((45 - AeInfo.LumY)/15);
	}

	Sensor_WriteReg(0x3052, OB_Value);
    ...
    //#NT#2011/12/01#Jarkko Chang -begin
    if (expoinfo.uiExpoRatio != 100)
    {
        AE_setFlag(AE_FLAG_SET_GAIN);
    }
    //#NT#2011/12/01#Jarkko Chang -end
...
{
    UINT32 tmp_ev = 0,tmp_lv = 0;

    Addr += sprintf((char *)Addr, "ExpectY = \t%ld\r\nAdd_Y = \t%ld\r\nLumY = \t%ld\r\nStableCnt = \t%ld\r\n",
                                            AEAlg_GetAdjExpectY(), AEAlg_GetAdjY(), AeInfo.LumY,AEAlg_GetStbCnt());

//#NT#2012/01/05#Jarkko Chang -begin
//#NT# special case for apc3
#if 1
    tmp_ev = AEAlg_GetPrvEVValue()-(AEAlg_GetPrvEVValue()/4);
    tmp_lv = AEAlg_GetPrvLV()- 5;
    Addr += sprintf((char *)Addr, "PrvEV_Value = \t%ld\r\nPrvLV = \t%ld\r\n",
                                            tmp_ev, tmp_lv);

    tmp_ev = AEAlg_GetCapEVValue()-(AEAlg_GetCapEVValue()/4);
    tmp_lv = AEAlg_GetCapLV()-5;
...
#endif
//#NT#2012/01/05#Jarkko Chang -end
...
#define LA_DETCRANGE_UPPER		0xff
#define LA_DETCRANGE_LOWER		0x10
...
UINT32 AE_getStatisticInfo(UINT32 HistSize, UINT32 *Hist)
{
	...
	for (j = 0; j < AE_WIN_Y; j ++)
    {
        ...
        {
			if(GAcc[j * AE_WIN_X + i] > LA_DETCRANGE_UPPER)
				GAcc[j * AE_WIN_X + i] = LA_DETCRANGE_UPPER;
			else if(GAcc[j * AE_WIN_X + i] < LA_DETCRANGE_LOWER)
				GAcc[j * AE_WIN_X + i] = LA_DETCRANGE_LOWER;
        }
	}
#endif
...
void AE_getCapTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_SCENE nScene, AE_ANTISHAKE nShakeLevel, AE_INFO *AeInfo)
{
    ...
    if (nShakeLevel != AE_ANTISHAKE_LV0)
    {
        if(nFlicker == AE_FLICKER_50HZ)
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_AntiShake_50Hz;
        }
        else
        {
            AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_AntiShake_60Hz;
        }
    }
...
AE_ISO AE_getIsoType(UINT32 Idx)
{
    ...
    if (Idx == _IMGISO_1600) return AE_ISO_1600;
    if (Idx == _IMGISO_3200) return AE_ISO_3200;
    return AE_ISO_AUTO;
}

...
static UINT32 EVTable[(Max_EvValue + 1)] = {13,16,18,20,25,31,35,40,50,63,71,79,100,
                                        ...
                                        635,800};
/*static UINT32 EVTable[(Max_EvValue + 1)] = {25,31,35,40,50,63,71,79,100,
                                        126,141,159,200,252,283,317,367,414,466,518,569,
                                        635,683,733,800};   */
...
void AE_Process (void)
{
    ...
    AE_setExpT(PrvNewAeArg.ExpoTime);
    //Sensor_SetISO(ISOGain4SmoothLum);
    if((PreviousExpoTime >= 16666) && (PrvNewAeArg.ExpoTime<=10000))
    {
    ...
    //sie_waitVD(1);
    //debug_err(("EV=%d, ET=%d, ISO=%d, ISO2=%d\r\n", AEAlg_GetEVValue(), PrvNewAeArg.ExpoTime, PrvNewAeArg.ISOGain, ISOGain4SmoothLum));
...
#if 0
    {
        ...
        sprintf(msg1,"ExpT %d ISO %d", (int) PrvNewAeArg.ExpoTime, (int)PrvNewAeArg.ISOGain);
        SenFP_ShowOSDString(msg1,30,25,3);

        sprintf(msg2,"LumY %d PrvLV %d", (int)AeInfo.LumY, (int)AEAlg_GetPrvLV());
    ...
    if (AEUartMsg == ENABLE)
    {
        AE_ERR("%3d %3d %8d %3d %7d, %3d, %d, %d %d\r\n", AeInfo.LumY, AEAlg_GetPrvLV(), AEAlg_GetPrvEVValue(),
                    PrvNewAeArg.ISOGain, PrvNewAeArg.ExpoTime, AEAlg_GetAdjExpectY(), PrvNewAeArg.Iris,AEAlg_GetStbCnt(),expoinfo.uiLine);
        debug_err(("----------------------------\r\n"));
    }
}
...
{
    //ImageAlgInfo *ImgInfo;
    if (0)//CapNewAeArg.ISOGain > 100)
    {
    *ExpT = CapNewAeArg.ExpoTime/2;
    *ISO = CapNewAeArg.ISOGain-50;
    }
    else
    {
    //if (CapNewAeArg.ISOGain > 400) CapNewAeArg.ISOGain = 400;

    //if ((CapNewAeArg.ExpoTime > 60000)&&(ImgInfo->Frequency == _POWERFreq_50Hz)) CapNewAeArg.ExpoTime = 60000;
    //if ((CapNewAeArg.ExpoTime > 50000)&&(ImgInfo->Frequency == _POWERFreq_60Hz)) CapNewAeArg.ExpoTime = 50000;
    if (CapNewAeArg.ExpoTime > 60000) CapNewAeArg.ExpoTime = 60000;
...
{
    Addr += sprintf((char *)Addr, "VER:0613 ExpectY = \t%ld\r\nAdd_Y = \t%ld\r\nLumY = \t%ld\r\n",
                                            AEAlg_GetAdjExpectY(), AEAlg_GetAdjY(), AeInfo.LumY);
...
void AE_getCapTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_SCENE nScene, AE_ANTISHAKE nShakeLevel, AE_INFO *AeInfo)
{
    ...
    if(nFlicker == AE_FLICKER_50HZ)
    {
        switch(nISO)
        {
            ...
            case AE_ISO_AUTO:
            default:
                AeInfo->CapCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_AntiFlicker50;
                break;
        }
    }
...
UINT32 GetFnoRatio(AEALG_IRIS_IDX Idx)
{
    ...
    FN = Lens_Aperture_GetFNO(iris,Lens_Zoom_GetSection());//FF lens
    return ( FN * FN / 10);
}
#if 0
#endif

...
ER IPL_ChgModeOffTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEO:
            //config sensor
            Sensor_WakeUp(0);
            //if((gImageAlgInfo.Vid_Resolution == _Resolution_1920x1080)&&(Obj->Mode == IPL_MODE_VIDEO))
            //    Sensor_ChgMode(SENSOR_MODE_1920X1080);
            //else
                Sensor_ChgMode(SENSOR_MODE_1280X960);
            //hook ipp isrcb func
            Isr_cb.SIEISR_CB = S5K3E2_sie_isrCB;
            Isr_cb.IPEISR_CB = S5K3E2_ipe_isrCB;
            Isr_cb.IMEISR_CB = S5K3E2_ime_isrCB;
...
ER IPL_ChgModePrvTo(IPL_OBJ *Obj)
{
    IPPCTRL_OBJ ippctrl_obj = {0};
    PImageAlgInfo pimginfo;
    IME_OUTFMT uiCapFmt = IMEOUT2_FMT422_2;
    ER er_code;
    pimginfo = UI_GetParameter();
    switch(Obj->Mode)
    {
...
ER IPL_ChgModeVidTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEOREC:
            ...
            if(gImageAlgInfo.Vid_Resolution == _Resolution_640x480P60)
                Sensor_ChgMode(SENSOR_MODE_640X480P60);
            else if(gImageAlgInfo.Vid_Resolution == _Resolution_320x240P120)
                Sensor_ChgMode(SENSOR_MODE_320X240P120);
            else if(gImageAlgInfo.Vid_Resolution == _Resolution_1920x1080)
                Sensor_ChgMode(SENSOR_MODE_1920X1080);
            else
                Sensor_ChgMode(SENSOR_MODE_1280X960);

...
//#NT#add image filter
ER ImageFilter_IQParam(UINT32 ImageFilterMode, IPE_IQINFO *IQInfo)
{
    IME_FILM_GRAIN_INFO FilmGrain;

    IQInfo->YCFIX_EFFECT_PARAM = ImgFilterTab[ImageFilterMode].YCFIX;
    IQInfo->EDGE_EFFECT_PARAM = ImgFilterTab[ImageFilterMode].EDGE;
    IQInfo->COLOR_EFFECT_PARAM = ImgFilterTab[ImageFilterMode].COLOR;
    IQInfo->INVERSE_EFFECT_PARAM = ImgFilterTab[ImageFilterMode].INVERSE;
    IQInfo->COLOR_PARAM.ColorGain.ChR = ImgFilterTab[ImageFilterMode].COLOR_INFOR->ChR;
    IQInfo->COLOR_PARAM.ColorGain.ChGr = ImgFilterTab[ImageFilterMode].COLOR_INFOR->ChGr;
    IQInfo->COLOR_PARAM.ColorGain.ChGb = ImgFilterTab[ImageFilterMode].COLOR_INFOR->ChGb;
    IQInfo->COLOR_PARAM.ColorGain.ChB = ImgFilterTab[ImageFilterMode].COLOR_INFOR->ChB;
    IQInfo->COLOR_PARAM_EXT = ImgFilterTab[ImageFilterMode].IPE_ColorExt;
    IQInfo->COLOR_PARAM.ColorOfs.ChR = 0;
    IQInfo->COLOR_PARAM.ColorOfs.ChGr = 0;
    IQInfo->COLOR_PARAM.ColorOfs.ChGb = 0;
    IQInfo->COLOR_PARAM.ColorOfs.ChB = 0;

    IQInfo->FuncUpdateSel = IPE_FUNC_ENABLE;
    IQInfo->FuncSel |= ImgFilterTab[ImageFilterMode].FuncEn;
    IQInfo->ParamUpdateSel |= (IPE_SET_YCFIXEFFECT|IPE_SET_EDGEEFFECT|IPE_SET_COLOREFFECT|IPE_SET_INVERSEEFFECT|IPE_SET_COLOR_GAIN|IPE_SET_COLOR_EXT);

    if (ImgFilterTab[ImageFilterMode].GNoise[0] == 1)
    {
        FilmGrain.fgEn = IME_FMGRN_ENABLE;
        FilmGrain.fgW0 = ImgFilterTab[ImageFilterMode].GNoise[1];
        FilmGrain.fgW1 = ImgFilterTab[ImageFilterMode].GNoise[2];
        FilmGrain.fgVal = ImgFilterTab[ImageFilterMode].GNoise[3];
    }
    else
    {
        FilmGrain.fgEn = IME_FMGRN_DISABLE;
        FilmGrain.fgW0 = ImgFilterTab[ImageFilterMode].GNoise[1];
        FilmGrain.fgW1 = ImgFilterTab[ImageFilterMode].GNoise[2];
        FilmGrain.fgVal = ImgFilterTab[ImageFilterMode].GNoise[3];
    }
    ime_chgParam((void *)&FilmGrain, IME_CHG_FILM_GRAIN);
    SRA_SIE_GammaLUT_Using = ImgFilterTab[ImageFilterMode].Gamma;
    //sra_setGammaTable(SRA_READBY_SIE, ImgFilterTab[ImageFilterMode].Gamma);
    sra_setGammaTable(SRA_READBY_SIE, SRA_SIE_GammaLUT_Using);
    if(ImgFilterTab[ImageFilterMode].EdgeMapParam != NULL)
        IQInfo->EDGE_PARAM = ImgFilterTab[ImageFilterMode].EdgeMapParam;
...
//#NT#2011/08/22#Bowen Li -begin
//#NT#add image filter
            ImageFilter_IQParam(pimginfo->ImageFilter, IQInfo);
//#NT#2011/08/22#Bowen Li -end
#if 0

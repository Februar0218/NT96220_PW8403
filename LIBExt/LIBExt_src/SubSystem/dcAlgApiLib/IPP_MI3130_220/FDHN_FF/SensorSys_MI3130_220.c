...
static UINT32 m_uiCapAECompensation = 100;
...
void Set2Preview(PIPPCTRL_INFO info, PIPPCTRL_RST result)
{
    ...
    debug_err((">>>>sensor mode = %d\r\n",SIE_PRE_Para_prv.ModeSel));
    ...
    SIE_PRE_Para_prv.SieIntEn = SIE_INTE_FLDEND|SIE_INTE_BP1|SIE_INTE_OVFL1;
    SIE_PRE_Para_prv.SieMaster = SlaveMode;
    SIE_PRE_Para_prv.SieVDPhase= 1;
    ...
    debug_err(("^RSet2Preview...DP_B=%d, DP_W=%d, ECS=%d...\r\n", CalInfo.CalStatus_PrvDP_B, CalInfo.CalStatus_PrvDP_W, CalInfo.CalStatus_ECS));
    if ((CalInfo.CalStatus_PrvDP_B == _Prv_B_Status) && (CalInfo.CalStatus_PrvDP_W == _Prv_W_Status))
    {
        debug_err(("Set2Preview...DPC...\r\n"));
...
void CaptureOne(SIE_PRE_PARAM *SiePreParam)
{
    ...
    if (GetIPPFuncInfor(DFS_EN))
    {
        ...
        SiePreParam->SiePathSel = PATH_PRE_COLORGAIN; // PATH_PRE_GAMMA;    [NTK#RandyHo]<2012/07/02>
...
void siepre_GetCapRaw(SIE_PRE_PARAM *SiePreParam)
{
    ...
    m_uiCapAECompensation = expoinfo.uiExpoRatio;
    Sensor_SetISO(capiso*expoinfo.uiExpoRatio/100);
    SENSORLIB_MSG("Cap AE: %d,%d %d\r\n",capiso,capext,m_uiCapAECompensation);
...
//bias warm color
void ColorGainAdjust(UINT32 *Rg,UINT32 *Gg, UINT32 *Bg)
{
    static BOOL bOutDoor = FALSE;
    SENSOR_STATUS SensorStatus;
    UINT32 lv_l = 105,lv_h = 120;
    PImageAlgInfo pimginfo;

    pimginfo = UI_GetParameter();
    Sensor_GetStatus(&SensorStatus);

    if(pimginfo->WBMode != _IMGWBMode_Auto)
    {
        return;
    }

    //cause Bayer RGB curve none linear under all lumi.
    (*Rg) = (*Rg)*255/(255-SensorStatus.pSyncInfo->OB_Offset/4);
    (*Bg) = (*Bg)*255/(255-SensorStatus.pSyncInfo->OB_Offset/4);

    if(AEAlg_GetStbCnt()>1)
    {
        //bias warm color for outdoor, according adpk request
        if(AEAlg_GetPrvLV() < lv_l)
        {
            bOutDoor = FALSE;
            return;
        }

        if(AEAlg_GetPrvLV()>= lv_h)
        {
            bOutDoor = TRUE;
            (*Rg) = (*Rg)*102/100;
            (*Bg) = (*Bg)*95/100;
...
void sie_PrvParameter(struct  PUBLIC_STR_JOBREQS *job_req)
{
    ...
    if(job_req->SIE_PRE_PRV->SieFuncEn & SIE_CG_EN)
    {
       ...
       // ColorGainAdjust(&Rg,&Gg,&Bg);  // delete this statement [NTK#RandyHo]<2012/05/24>
...
void sie_CapParameter(struct  PUBLIC_STR_JOBREQS *job_req)
{
    ...
    if(job_req->SIE_PRE_CAP->SieFuncEn & SIE_CG_EN)
    {
        ...
        //Accroding to color checker RGB channels adjustment in photoshop [NTK#RandyHo]<2012/05/25>
        Rg += 30;
...
void Write_AE_EXIF_Infor(UINT paddress)
{
    ...
    static UINT32 fn[2] = {30,10};
    static UINT32 shutterspeed[2];
    static UINT32 exptime[2];
    static UINT32 focus[2] = {50,10};
    ...
    // begin-re-write from if-else-if to switch()... [NTK-SZ#RandyHo]<2012/03/08>
    ...
    */
    // end-re-write to switch()... [NTK-SZ#RandyHo]<2012/03/08>
    ...
    modifyTag.ptagDataAddr = (UINT32 *)fn;
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version: FDHN_MI3130_FF_20120725 at adpk by [NTK#RandyHo] %s",endl);
    *buff++= '\t';
    ...
    */
    ...
    buff += sprintf((char *)buff,"\t\t Status:   ISO:0x%x, AWB:0x%x, AWB_GS:0x%x, DP_PrvB:0x%x, DP_PrvW:0x%x, DP_CapB:0x%x, DP_CapW:0x%x, ECS:0x%x OB: 0x%x%s",
    ...
    {
        buff += sprintf((char *)buff,"\t\t ISO:   %3d%s", CalInfo.Cal_ISO_Data[0], endl);
    }

    if(CalInfo.CalStatus_AWB == _AWB_Status)
    {
        buff += sprintf((char *)buff,"\t\t AWB:   %3d %3d%s", CalInfo.Cal_AWBGain.Rgain, CalInfo.Cal_AWBGain.Bgain, endl);
    }

    if(CalInfo.CalStatus_AWBGS == _AWB_Status)
    {
        buff += sprintf((char *)buff,"\t\t AWBGS:   %3d %3d%s", CalInfo.Cal_AWBGSGain.Rgain, CalInfo.Cal_AWBGSGain.Bgain, endl);
    }
    ...
    */
    ...
    buff += sprintf((char *)buff,"%s AE IsoCompensationRatio \t%d %s", endl,(UINT)m_uiCapAECompensation,endl);
    for(  i = 0; i < 80; i++ )
    {
        *buff++ = '-' ;
    }
...
void SetSiePreCapParam(SIE_PRE_PARAM *SiePreInfo)
{
    ...
    SiePreInfo->SieHDPhase = 1;
    SiePreInfo->SieVDPhase = 1;

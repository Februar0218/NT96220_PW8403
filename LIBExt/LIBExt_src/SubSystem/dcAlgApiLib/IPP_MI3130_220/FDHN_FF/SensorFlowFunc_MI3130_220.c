/*
    ...
    @file       SensorFlowFunc_MI3130_220.c
...
#define IPE_CC_TYPE     NORMAL_CC   // [NTK#RandyHo]<2012/05/05>
...
ER ipe_IQparam(UINT32 ParamMode, IPE_IQINFO *IQInfo)
{
    ...
    case IPE_CAP_SETTING :
        ...
        // IQInfo->COLOR_EFFECT_PARAM->SatOfs += 20;   //37     [NTK#RandyHo]<2012/05/25>
    ...
    case IPE_SATURATION:
        if ( Is_AELock() == TRUE )
        {
           ...
           // IQInfo->COLOR_EFFECT_PARAM->SatOfs += 100; // increase saturation [NTK#RandyHo]<2012/05/24>
            //#NT#2011/09/08#Ethan Lau -end
            IQInfo->HueTab = IPE_hueTab;
            IQInfo->SatTab = IPE_satTab;
            IQInfo->IntTab = IPE_intTab;

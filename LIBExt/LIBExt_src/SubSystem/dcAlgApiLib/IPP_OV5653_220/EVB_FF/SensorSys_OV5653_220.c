...
#include "IQSetting_OV5653_220.h"
...
void Write_AE_EXIF_Infor(UINT paddress)
{
    ...
    {
        // step 0.3
        expbias[0] = gImageAlgInfo.AEEVValue-6;
        expbias[1] = 3;
    }
    else
    {
        // step 0.3
        expbias[0] = (gImageAlgInfo.AEEVValue-6)*2-3+AEB_Idx*3;
        expbias[1] = 6;
        AEB_Idx++;
    }
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version: HDCF_MI5130_FF_20110729    %s",endl);
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
    buff += sprintf((char *)buff,"%s", endl);

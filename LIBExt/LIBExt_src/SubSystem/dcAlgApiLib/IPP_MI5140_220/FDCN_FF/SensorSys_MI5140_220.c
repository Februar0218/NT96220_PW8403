...
#include "IQSetting_MI5140_220.h"
...
//#NT#fine tune cap-preview performance
    ...
    sie_waitVD(1);
    debug_err((">>>Set2Preview clear Ime flag\r\n"));
    if(bFirst)
    {
...
*/
    ...
    {
        if (AEAlg_GetPrvLV() < 45) 
        job_req->sra_gammaLUT = SRA_SIE_GammaLUT_Dark;   
        else 
        job_req->sra_gammaLUT = SRA_SIE_GammaLUT_Cap;
    }
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version:_MI5140_FF_20120716%s",endl);
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
    buff += sprintf((char *)buff,"%s", endl);

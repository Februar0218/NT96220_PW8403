...
ER IPL_ChgModeMovie(IPL_OBJ *Obj, BOOL bRec)
{
    if(bRec)
    {
        if(Obj->uiImeOutHeight2 > 720)
            pll_setClockRate(PLL_CLKSEL_SIE_MCLKDIV, PLL_SIE_MCLKDIV(39));//59
        else
            pll_setClockRate(PLL_CLKSEL_SIE_MCLKDIV, PLL_SIE_MCLKDIV(29));
    }
    else //prev
    {
            pll_setClockRate(PLL_CLKSEL_SIE_MCLKDIV, PLL_SIE_MCLKDIV(29));
...
ER IPL_ChgModeOffTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEO:
            //config sensor
            Sensor_WakeUp(0);
            Sensor_ChgMode(SENSOR_MODE_1280X960);
            //hook ipp isrcb func
            Isr_cb.SIEISR_CB = MI5140_sie_isrCB;
            Isr_cb.IPEISR_CB = MI5140_ipe_isrCB;
            Isr_cb.IMEISR_CB = MI5140_ime_isrCB;
...
ER IPL_ChgModeVidTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEOREC:
            ...
            IPPCTRL_SetCmd(ippctrl_obj);
            //config sensor
            //Sensor_ChgMode(SENSOR_MODE_1280X960);//ethan_20110808_mark, same resolution with preview
            ...
            jpgobj.TriggerIdx = 0;//speed up chg mode
            Dcout_Setcmd(&jpgobj);
            //config Image pipeline
...
ER IPL_ChgModeRecTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        case IPL_MODE_VIDEO:
            ...
            Dcout_Setcmd(&jpgobj);
            //config sensor
            //Sensor_ChgMode(SENSOR_MODE_1280X960);same resolution with preview
            //config Image pipeline
            ippctrl_obj.fpIppFlowApi = Set2Preview;
            ippctrl_obj.fpStrongFlow = NULL;
            ippctrl_obj.IPPctrl_info.uiDzoomStep = Obj->uiDzoomStep;

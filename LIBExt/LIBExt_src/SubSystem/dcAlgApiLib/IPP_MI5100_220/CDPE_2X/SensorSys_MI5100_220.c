...
#include "IQSetting_MI5100_220.h"
#define SENSORLIB_DEBUG  0
...
//    PRV_SIE_BP2(SIE_PRE_Para_prv.ModeSel,&SIE_PRE_Para_prv.SieBp2);
    ...
    if(CalInfo.CalStatus_ECS == _ECS_Status)
    {
        SIE_PRE_Para_prv.SieFuncEn |= SIE_ECS_EN;
         ...
         // -begin-input  ecs table corresponding  to sections of 2x lens [NTK#RandyHo]<2012/06/16>
        // ippbuf_SetBuf(IPPBUF_ECS, CalInfo.Cal_ECS_Addr);
       // ippbuf_SetBuf(IPPBUF_ECS, ippbuf_GetBuf(IPPBUF_ECS)+(Lens_Zoom_GetSection()-1)*ECS_TblSize);

         ippbuf_SetBuf(IPPBUF_ECS, CalInfo.Cal_ECS_Addr+(Lens_Zoom_GetSection()-1)*ECS_TblSize);

        SENSORLIB_MSG("^RECS table addr: %x\r\n", ippbuf_GetBuf(IPPBUF_ECS));
        // -end-input  ecs table corresponding  to sections of 2x lens [NTK#RandyHo]<2012/06/16>
    }
...
void Set2Video(PIPPCTRL_INFO info, PIPPCTRL_RST result)
{
    ...
    {
        SIE_PRE_Para_prv.SiePreAddr2 = CalInfo.Cal_DP_VideoAddr;
        SIE_PRE_Para_prv.SieFuncEn |= SIE_BLM_EN;
    }
    if(CalInfo.CalStatus_ECS == _ECS_Status)
    {
...
void siepre_GetCapRaw(SIE_PRE_PARAM *SiePreParam)
{
    ...
    if((CheckFlashFireStatus() != FALSE) && (FlashParam->FakeFlash != 0))
    {
...
void sie_PrvParameter(struct  PUBLIC_STR_JOBREQS *job_req)
{
    ...
    if (job_req->SIE_PRE_PRV->SieFuncEn & SIE_ECS_EN)
    {
        ...
        job_req->SIE_PRE_PRV->SiePreAddr1= ippbuf_GetBuf(IPPBUF_ECS); // for 2x(3 sections) ecs calibration [NTK#RandyHo]<2012/06/15>
...
void sie_CapParameter(struct  PUBLIC_STR_JOBREQS *job_req)
{
    ...
    if (job_req->SIE_PRE_CAP->SieFuncEn & SIE_ECS_EN)
    {
       ...
       // job_req->SIE_PRE_CAP->SiePreAddr1= ippbuf_GetBuf(IPPBUF_ECS)+(zoomSection-1)*ECS_TblSize;   // for 2x(3 sections) ecs calibration [NTK#RandyHo]<2012/06/15>
    }
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version: MI5100_2X 20120808 by [NTK#RandyHo] %s",endl);
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
    buff += sprintf((char *)buff,"%s", endl);
    ...
    */
    ...
    {
        // -begin-input  ecs table corresponding  to sections of 2x lens [NTK#RandyHo]<2012/06/16>
        buff += sprintf((char *)buff,"\t\t ECS Table[%ld] of [%ld], addr[0x%lx]", Lens_Zoom_GetSection(), Lens_Zoom_GetMaxSection(), ippbuf_GetBuf(IPPBUF_ECS)+(Lens_Zoom_GetSection()-1)*ECS_TblSize);

         // buff += sprintf((char *)buff,"\t\t ECS:   0x%x", CalInfo.Cal_ECS_Addr);     // old style for one ecs table only(FF)
        // -end-input  ecs table corresponding  to sections of 2x lens [NTK#RandyHo]<2012/06/16>
    }

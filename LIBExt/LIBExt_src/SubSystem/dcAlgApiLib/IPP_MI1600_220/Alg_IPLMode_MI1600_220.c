...
//#define TEST_MODE   SENSOR_MODE_1440X1080
//#define TEST_MODE   SENSOR_MODE_1280X720
//#define TEST_MODE   SENSOR_MODE_640X480
//#define TEST_MODE   SENSOR_MODE_1440X540


ER IPL_ChgModePhoto(IPL_OBJ *Obj, BOOL bCap)
{
    if(bCap)
    {
        debug_err(("^RUnknown Photo-Cap Mode!\r\n"));
    }
    else
    {
        #ifndef TEST_MODE
        Sensor_ChgMode(SENSOR_MODE_1440X540);
        #else
        Sensor_ChgMode(TEST_MODE);
        #endif
    }
    return E_OK;
}

ER IPL_ChgModeMovie(IPL_OBJ *Obj, BOOL bRec)
{
    if(bRec)
    {
        #ifndef TEST_MODE
        if(gImageAlgInfo.Vid_Resolution == _Resolution_1920x1080)
            Sensor_ChgMode(SENSOR_MODE_1440X1080);
        else
        if(gImageAlgInfo.Vid_Resolution == _Resolution_1280x720)
            Sensor_ChgMode(SENSOR_MODE_1280X720);
        else
        if(gImageAlgInfo.Vid_Resolution == _Resolution_640x480)
            Sensor_ChgMode(SENSOR_MODE_1440X540);
        else
            debug_err(("^RUnknown Video-Rec Mode!\r\n"));
        #else
            Sensor_ChgMode(TEST_MODE);
        #endif
    }
    else //prev
    {
        ...
        if(gImageAlgInfo.Vid_Resolution == _Resolution_640x480)
            ...
            debug_err(("^RUnknown Video-Prev Mode!\r\n"));
        #else
            Sensor_ChgMode(TEST_MODE);
        #endif
    }
...
{
    ER rt;
    static char StName[IPL_MODE_NUM][10]=
    {
    "OFF",
    "IDLE",
    "PREV",
    ...
    if (rt == E_OK)
    {
        debug_msg("IPL chg mode from %s to %s\r\n", StName[gIPLCurObj.Mode], StName[Obj->Mode]);
    ...
    {
        debug_err(("^RIPL chg mode fail (from %s to %s)\r\n",StName[gIPLCurObj.Mode], StName[Obj->Mode]));
    }
    return rt;
}
...
ER IPL_ChgModeOffTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        case IPL_MODE_PREVIEW:
            //config sensor
            Sensor_WakeUp(0);
            debug_err(("IPP off->preview\r\n"));
            debug_err(("IME p1(%d,%d) p2(%d,%d)\r\n", Obj->uiImeOutWidth1, Obj->uiImeOutHeight1, Obj->uiImeOutWidth2, Obj->uiImeOutHeight2));
            IPL_ChgModePhoto(Obj, FALSE);
            //hook ipp isrcb func
            Isr_cb.SIEISR_CB = MI1600_sie_isrCB;
            Isr_cb.IPEISR_CB = MI1600_ipe_isrCB;
            Isr_cb.IMEISR_CB = MI1600_ime_isrCB;
        ...
        case IPL_MODE_VIDEO:
            //config sensor
            Sensor_WakeUp(0);
            debug_err(("IPP off->video\r\n"));
            debug_err(("IME p1(%d,%d) p2(%d,%d)\r\n", Obj->uiImeOutWidth1, Obj->uiImeOutHeight1, Obj->uiImeOutWidth2, Obj->uiImeOutHeight2));
            IPL_ChgModeMovie(Obj, FALSE);
            //hook ipp isrcb func
            Isr_cb.SIEISR_CB = MI1600_sie_isrCB;
            Isr_cb.IPEISR_CB = MI1600_ipe_isrCB;
            Isr_cb.IMEISR_CB = MI1600_ime_isrCB;
        ...
        case IPL_MODE_PCC:
            //config sensor
            Sensor_WakeUp(0);
            debug_err(("IPP off->pcc\r\n"));
            debug_err(("IME p1(%d,%d) p2(%d,%d)\r\n", Obj->uiImeOutWidth1, Obj->uiImeOutHeight1, Obj->uiImeOutWidth2, Obj->uiImeOutHeight2));
            Sensor_ChgMode(SENSOR_MODE_640X480);
...
ER IPL_ChgModeVidTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEOREC:
            ...
            IPPCTRL_SetCmd(ippctrl_obj);
            //config sensor
            debug_err(("IPP video->rec\r\n"));
            debug_err(("IME p1(%d,%d) p2(%d,%d)\r\n", Obj->uiImeOutWidth1, Obj->uiImeOutHeight1, Obj->uiImeOutWidth2, Obj->uiImeOutHeight2));
            IPL_ChgModeMovie(Obj, TRUE);

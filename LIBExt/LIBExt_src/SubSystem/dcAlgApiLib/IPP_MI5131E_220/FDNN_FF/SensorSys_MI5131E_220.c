...
#define G_offset    0
#endif
#define FDNN_SpecialCase    ENABLE//cause color shading unstable over 8x analog gain
#if FDNN_SpecialCase
#define MaxColorGain    (256*63/8)      //assume max is 7.875x
UINT32  GainRatio = 100;
#endif
#define Get10BitRAW DISABLE
#define Get8BitRAW DISABLE  //if FDNN_SpecialCase == ENABLE, 8bitraw must disable
...
#include "IQSetting_MI5131E_220.h"
...
#if FDNN_SpecialCase
            SiePreParam->SieFuncEn |= SIE_CG_EN; //enable,use digital gain to replace analog gain
            SiePreParam->SiePathSel = PATH_PRE_GAMMA;
#endif
...
void siepre_GetCapRaw(SIE_PRE_PARAM *SiePreParam)
{
...
#if FDNN_SpecialCase
    GainRatio = 100;
    AE_GetPrvAEArg(&prviso,&prvexpt,&iris);
    if(capiso !=prviso )
    {
        GainRatio = GainRatio*capiso/prviso;
        capiso = prviso;
    }

    AWB_GetColorGian(&R,&G,&B);
    Max = (R>G?R:G);
    Max = (Max>B?Max:B);
    if( (Max * GainRatio/100)> MaxColorGain)  //color gain limitation is clampped at 7.5x
    {
        capiso = capiso* GainRatio*Max/(MaxColorGain*100);//capiso* GainRatio/(MaxColorGain*100/Max);
        GainRatio = MaxColorGain*100/Max;
    }
    //updata digital gain
    ...
    //#PIC#2012/07/04#david -begin
    if(g_fpAlgExport)
    {
        g_fpAlgExport(ALGMSG_SHUTTERSOUND, &gAlgMsg);
    }
    //#PIC#2012/07/04#david -end
    ...
    if(job_req->SIE_PRE_PRV->SieFuncEn & SIE_CG_EN)
    {
        ...
        switch(job_req->SIE_PRE_PRV->SieCfaPat)
        {
            case IPE_PATR:
                ...
                job_req->sie_cofs_info.Ofs1 = G_offset;
                job_req->sie_cofs_info.Ofs2 = G_offset;
            ...
            case IPE_PATGR:
                ...
                job_req->sie_cofs_info.Ofs0 = G_offset;
                job_req->sie_cofs_info.Ofs1 = 0;
                job_req->sie_cofs_info.Ofs2 = 0;
                job_req->sie_cofs_info.Ofs3 = G_offset;
                break;
            case IPE_PATGB:
                job_req->sie_cg_info.Gain0 = Gg;
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version: FDNN_MI5131E_FF_20111201_IQV3    %s",endl);
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
    buff += sprintf((char *)buff,"%s", endl);
    ...
    */
#if FDNN_SpecialCase
    buff += sprintf((char *)buff,"AWB CapCA: %s",endl);
#endif
    buff = (unsigned char *)AWB_WriteDebugInfo((UINT32)buff);
    {
        ...
        Gg = Gg*GainRatio/100;
        Bg = Bg*GainRatio/100;
        buff += sprintf((char *)buff,"Gain Ratio: %d %s",(UINT)GainRatio,endl);
#endif
        buff += sprintf((char *)buff,"AWB Real Gain: %d %d %d%s",(UINT)Rg,(UINT)Gg,(UINT)Bg, endl);
    }
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
...
#if FDNN_SpecialCase
    SetIPPFuncInfor(SIED2D_EN,ON);//for cap awb
    UI_SetParameter(_SieCapBitDepth, _SieBitDepth_10);
#endif

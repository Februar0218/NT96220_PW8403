...
#if FDNN
{
            ...
            if(r<220 && b>650)
            {
                IQInfo->COLOR_EFFECT_PARAM->SatOfs =  IQInfo->COLOR_EFFECT_PARAM->SatOfs*12/10;
            }
}
#endif
...
void SenFlowFunc(SEN_FLOW_MSG MsgID)
{
    ...
    switch(MsgID)
    {
        ...
        case AWB_CAP_SETTING:
            if(1)
            {
                UINT32 addr = ippbuf_GetBuf(IPPBUF_CAP_RAW_1);
                ...
                if(GetIPPFuncInfor(IFE_RAW_EN))
                {
                    ...
                    addr += (IFE_OVERLAP_H * ifeRepeat);
                }
                AWB_ReCalColorGain(addr,GetIPPSizeInfor(_SieOutLineOffset),
                    GetIPPSizeInfor(_SieCropCapHsize),GetIPPSizeInfor(_SieCropCapVsize),
                    pimginfo->SieCapInBitDepth,8,SIE_PRE_Para_cap.SieCfaPat);
            }
            break;

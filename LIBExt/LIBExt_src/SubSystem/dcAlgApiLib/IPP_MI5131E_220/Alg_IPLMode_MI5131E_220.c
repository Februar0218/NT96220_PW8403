...
ER IPL_ChgModeOffTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEO:
            ...
            //hook ipp isrcb func
            Isr_cb.SIEISR_CB = MI5131E_sie_isrCB;
            Isr_cb.IPEISR_CB = MI5131E_ipe_isrCB;
            Isr_cb.IMEISR_CB = MI5131E_ime_isrCB;
...
ER IPL_ChgModeVidTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        ...
        case IPL_MODE_VIDEOREC:
            ...
            IPPCTRL_SetCmd(ippctrl_obj);
            //config sensor
            //Sensor_ChgMode(SENSOR_MODE_FULL);//ethan_20110808_mark, same resolution with preview
            //#NT#2011/12/20#Ethan Lau -begin
            //#NT#chg to full resotluion if outputsize is 1080P (10fps) so far
            if( (Obj->uiImeOutWidth2 > 1440)&& (Obj->uiImeOutHeight2 > 960) )
...
ER IPL_ChgModeRecTo(IPL_OBJ *Obj)
{
    ...
    switch(Obj->Mode)
    {
        case IPL_MODE_VIDEO:
            ...
            Dcout_Setcmd(&jpgobj);
            //config sensor
            //#NT#2011/12/21#Ethan Lau -begin
            //#NT#vidRec. maybe use Full Mode
            Sensor_ChgMode(SENSOR_MODE_1280X960);
            //#NT#2011/12/21#Ethan Lau -end
            //config Image pipeline
            ippctrl_obj.fpIppFlowApi = Set2Preview;
            ippctrl_obj.fpStrongFlow = NULL;
            ippctrl_obj.IPPctrl_info.uiDzoomStep = Obj->uiDzoomStep;

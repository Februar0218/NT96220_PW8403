...
extern INT32 InterpolateValue(UINT32 Index, UINT32 LowValue, UINT32 HighValue, UINT32 MinIndex, UINT32 MaxIndex);
...
//#NT#2012/04/30#Spark Chou -begin
...
UINT32 IfeCCOft(UINT32 ParamMode, UINT32 iso)
{
    UINT32 value;
    if ( ParamMode== IPE_PRV_SETTING || IPE_PRV_AE_SETTING )
        value = 0;
    else
        value = (IFE_Func[iso][IFE_YCC]==ENABLE ? IFE_param_ycc[iso][0].repeat:0);
    return value;
}
//#NT#2011/11/17#Ethan Lau -end
//#NT#2012/04/30#Spark Chou -end
extern UINT32 sie_gamma_tbl[65];
extern UINT8 bGammaTrigger;
...
#if 1
	if(ISO > AUTO_GAMMA_ISO_LOWER)
	{
		auto_gamma_weight = (ISO - AUTO_GAMMA_ISO_LOWER);

		if(auto_gamma_weight > AUTO_GAMMA_ISO_RANGE)
			auto_gamma_weight = AUTO_GAMMA_ISO_RANGE;
	}
	else
		auto_gamma_weight = 0;
#else

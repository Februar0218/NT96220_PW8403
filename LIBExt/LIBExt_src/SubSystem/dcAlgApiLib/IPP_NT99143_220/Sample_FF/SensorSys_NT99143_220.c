...
#include "IQSetting_NT99143_220.h"
...
#define SENSORLIB_VERSION    "2014/12/103_V1.2"
...
const UINT32 CapBufAlloc[Max_BurstNum][3]=
{
    ...
    {0x03, 0x03, 0x05},  //_BurstNum_5
    {0x03, 0x03, 0x06},  //_BurstNum_6
...
UINT32 WriteDebugData(unsigned char *Add)
{
    ...
    *buff++= '\t';
    ...
    buff += sprintf((char *)buff,"\t Version:%s     %s",SENSORLIB_VERSION, endl);
    *buff++= '\t';
    for(  i = 0; i < 80; i++ )
        *buff++ = '-' ;
    buff += sprintf((char *)buff,"%s", endl);
...
void SetSiePreCapParam(SIE_PRE_PARAM *SiePreInfo)
{
    ...
    //SiePreInfo->SiePathSel = PATH_PRE_GAMMA;ae
#if Get10BitRAW

...
INT32 MenuCommonItem_OnKeyDisplay(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_KEY_DISPLAY,MenuCommonItem_OnKeyDisplay)
...
INT32 MenuCommonItem_OnKeyDisplay(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32  uiKeyAct;
 ...
 UINT32  uiKeyAct;
   ...
   uiKeyAct = paramArray[0];
    ...
    else if (pMenu->Status == TMS_ON_TAB)
    {
       ...
       UxMenu_SetData(pCtrl,MNU_CURITM,pPage->Count-1);   
       for (i = 0; i < pPage->Count; i++)
            UxMenu_SetItemData(&MenuCommonItem_MenuCtrl, i, MNUITM_STATUS, STATUS_ENABLE); // enable all items (for page info calculating)

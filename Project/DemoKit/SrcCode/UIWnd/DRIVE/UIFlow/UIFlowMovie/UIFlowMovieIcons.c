...
void FlowMovie_IconDrawShrinkRecord(VControl *pCtrl)
{
	if (SysGetFlag(FL_MOVIE_TIMESHRINK) == MOVIE_TIMESHRINK_ON)
	{
		UxState_SetData(pCtrl,STATE_CURITEM,UIFlowWndMovie_Status_SHRINK_ICON_DATE_TIME);
    		UxCtrl_SetShow(pCtrl, TRUE);
	}
	else
	{
		UxState_SetData(pCtrl,STATE_CURITEM,UIFlowWndMovie_Status_SHRINK_ICONID_NULL);
    		UxCtrl_SetShow(pCtrl, FALSE);
	}
}
...
void FlowMovie_IconDrawDaoche(void)
{
    UxCtrl_SetShow(&UIFlowWndMovie_Static_DAOCHECtrl, FALSE);
}

void FlowMovie_IconHideDaoche(void)
{
    UxCtrl_SetShow(&UIFlowWndMovie_Static_DAOCHECtrl, FALSE);
}
...
void FlowMovie_UpdateIcons(BOOL bShow)
{
    ...
    else
    {
        ...
        FlowMovie_IconDrawLedLight(&UIFlowWndMovie_Status_LedLightCtrl);	//eric edit 0901	
	  ...
	  FlowMovie_IconHideYellowRec(&UIFlowWndMovie_PanelYellowCtrl);
	  FlowMovie_IconDrawShrinkRecord(&UIFlowWndMovie_Status_SHRINKCtrl);

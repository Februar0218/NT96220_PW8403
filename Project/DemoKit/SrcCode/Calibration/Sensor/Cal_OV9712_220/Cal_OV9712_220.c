...
/*
    ...
    */
    ...
    {
        Cal_SetCalData(CAL_DATANUM_STATUS_ISO,  (UINT*)&AAA_Adjust.ISO_Status);
        Cal_SetCalData(CAL_DATANUM_ISO,  (UINT*)&AAA_Adjust.Agc_ISO_Base);
    }
...
UINT32 Mshutter_Adjust(void)
{
    ...
    UINT32 PrvMode = SENSOR_MODE_FULL;
...
UINT32 AWB_Adjust(void)
{
    ...
    UINT32 PrvMode = SENSOR_MODE_FULL;
    FLGPTN  uiFlag = 0;
    UINT32 PrvBinning = 1;  //Prv no binning
    ...
    AWBParam.uiOBofs = 0x1;
    AWBParam.uiMsht = 31500;
    AWBParam.bSaveRaw = FALSE;
    AWBParam.uiExpT = (AeSetting.uiExpT * PrvBinning);
...
//    UINT32 AGCGain;
    ...
    Cal_ShowStringWithColor("Face LV10 and Press SHUTTER",20, 80, 3);
    ...
    ISOParam.uiPrvMode = SENSOR_MODE_FULL;
    ISOParam.uiTarget = 60;
    ISOParam.uiTime = 16666;
    ISOParam.uiGain = 1000;
    ISOParam.uiAdjStep = 100;
    ISOParam.uiISOBase = 50;
    ...
    if(ISOParam.uiISOBaseGain <1)
        ErrorCode = CAL_ERR_ISO_MIN;
    else if(ISOParam.uiISOBaseGain >10000)
    ...
    AAA_Adjust.Agc_ISO_Base = ISOParam.uiISOBaseGain;

    sprintf(CalStringBuffer,"ISO gain %d",(UINT)AAA_Adjust.Agc_ISO_Base);
...
UINT32 DP_Adjust_White(void)
{
    ...
    DPCParam.uiOBofs = 0x1;
    DPCParam.uiMsht = 31500;
    DPCParam.bSaveRaw = FALSE; //FALSE: Do not save raw, TRUE: save raw
    DPCParam.uiTarget = 150;
...
ER Cal_ShowCalRst(void)
{
    ...
    if(AAA_Adjust.ECS_Status == _ECS_Status)
        sprintf(CalStringBuffer,"ECS : OK");
    else
        sprintf(CalStringBuffer,"ECS : NG");
    Cal_ShowStringWithColor(CalStringBuffer,20, 60, 4);
